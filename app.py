...
#             with col_r:
#                 # 💡 [수정됨] st.metric 대신 시인성이 높은 커스텀 상태 카드로 교체
#                 score = d['panel_1_visual']['map50_score']
#                 color = "#28a745" if score >= 0.70 else "#ff4b4b" # 0.70 이상 초록색, 미만 빨간색
#                 status_text = "✅ 정상 (PASSED)" if score >= 0.70 else "⚠️ 위험 (FAILED)"
#                 delta = score - 0.70
                
#                 st.markdown(f"""
#                     <div style='background-color: #111827; padding: 20px; border-radius: 10px; border: 1px solid #374151; border-left: 8px solid {color}; margin-bottom: 20px;'>
#                         <p style='margin:0; color:#9ca3af; font-size:1.1em;'>mAP50 Score</p>
#                         <h1 style='margin:0; color:{color}; font-size: 2.8em;'>{score:.4f}</h1>
#                         <p style='margin:0; color:{color}; font-weight:bold;'>{status_text} 
#                             <span style='color:#9ca3af; font-weight:normal; font-size:0.9em;'>(목표치 대비 {delta:+.4f})</span>
#                         </p>
#                     </div>
#                 """, unsafe_allow_html=True)
...
#                 fig.add_hline(y=0.70, line_dash="dash", line_color="red", annotation_text="Safety Line (70%)")
...
if mode == "실시간 모니터링 (Live)":
    ...
    if st.session_state.live_active:
        p_status, p_img, p_info = st.empty(), st.empty(), st.empty()
        ...
        while st.session_state.live_active:
            ...
            if img_files:
                ...
                if data:
                    # 💡 동적 Safety Line 적용
                    safety_line = data.get("safety_line", 0.70)
                    score = data["panel_1_visual"]["map50_score"]
                    color = "#28a745" if score >= safety_line else "#ff4b4b"
                    status_text = "✅ 정상 (PASSED)" if score >= safety_line else "⚠️ 위험 (Safety Line 붕괴)"
                    
                    p_status.markdown(f"""
                        <div class='status-card' style='border-left: 10px solid {color};'>
                            <h2 style='margin:0; color:{color};'>최종 신뢰도: {score:.4f}</h2>
                            <p style='margin:0; font-size:1.1em;'>상태: <b>{status_text}</b> 
                            | Step: {latest_step}/5 | <span style='color:#9ca3af;'>기준선: {safety_line:.4f}</span></p>
                        </div>
                    """, unsafe_allow_html=True)
                    with p_info.container():
                        st.info(f"**반사실 시나리오 생성 가설:** {data['panel_3_llm']['hypothesis']}")
...
# 메인 화면 - 히스토리 분석
    ...
    if not steps:
        ...
        if d:
            ...
            with col_r:
                # 💡 동적 Safety Line을 적용한 커스텀 카드 렌더링
                safety_line = d.get("safety_line", 0.70)
                score = d['panel_1_visual']['map50_score']
                color = "#28a745" if score >= safety_line else "#ff4b4b"
                status_text = "✅ 정상 (PASSED)" if score >= safety_line else "⚠️ 위험 (FAILED)"
                delta = score - safety_line
                ...
                st.markdown(f"""
                    <div style='background-color: #111827; padding: 20px; border-radius: 10px; border: 1px solid #374151; border-left: 8px solid {color}; margin-bottom: 20px;'>
                        ...
                        <h1 style='margin:0; color:{color}; font-size: 2.8em;'>{score:.4f}</h1>
                        <p style='margin:0; color:{color}; font-weight:bold;'>{status_text} 
                            <span style='color:#9ca3af; font-weight:normal; font-size:0.9em;'>(기준선 {safety_line:.4f} 대비 {delta:+.4f})</span>
                        </p>
                    </div>
                """, unsafe_allow_html=True)
            ...
            st.divider()
            history_data = []
            dynamic_sl = 0.70 # 기본값 폴백
            
            for s in steps:
                hist_d = load_json(s)
                if hist_d:
                    history_data.append({"Step": s, "mAP": hist_d["panel_1_visual"]["map50_score"]})
                    dynamic_sl = hist_d.get("safety_line", dynamic_sl) # 저장된 동적 기준선 획득
            
            if history_data:
                df_hist = pd.DataFrame(history_data)
                fig = px.line(df_hist, x="Step", y="mAP", markers=True, title="성능 하락 타임라인 (mAP50)")
                # 💡 빨간 점선을 동적 기준선 높이에 맞게 렌더링
                fig.add_hline(y=dynamic_sl, line_dash="dash", line_color="red", annotation_text=f"Dynamic Safety Line ({dynamic_sl:.4f})")
                fig.update_layout(yaxis_range=[0, 1])
                st.plotly_chart(fig, use_container_width=True)