...
for iter = 1:maxIter
    ...
    % Switch to DSPy adapter: change 'dspy_for_simulink' → 'gpt_for_simulink' to revert.
    llm_module = "llm_agent.dspy_for_simulink";
    cmd = sprintf('%s -m %s --input "%s" --output "%s" --iter_tag "%s"%s', ...
        char(opts.python_exe), char(llm_module), ...
