...
#     print(f"\n🎯 [탐색 완료] 총 3단계의 동적 변조 및 능동 탐색 루프를 성공적으로 마쳤습니다.")
...
def run_dynamic_pipeline():
    s_print("🚀 UAV 자율 검증 파이프라인 가동")
    
    # 모델 로딩 (서버 GPU 사용)
    llm = GPTGenerator(model_name="gpt-5.4-nano")
    xai = RealXAIAnalyzer() # 여기서 yolo11x.pt 로드
    
    current_state = {"map50": 0.95, "xai_signals": {"dominant_factors": []}}
    ...
    for step in range(1, 4):
        ...
        aug = A.Compose([
            A.RandomFog(alpha_coef=f/100.0, p=1.0 if f>0 else 0),
            A.GaussNoise(p=min(n*10, 1.0) if n>0 else 0),
            A.MotionBlur(blur_limit=(3, max(3, m if m%2!=0 else m+1)), p=1.0 if m>2 else 0)
        ])
        degraded_img = aug(image=base_img)['image']
        ...
        current_state = {"map50": new_map50, "xai_signals": {"dominant_factors": feature_importance}}
