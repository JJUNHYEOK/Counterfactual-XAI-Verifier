...
main = uigridlayout(fig, [2, 1], ...
    "RowHeight",   {'1x', 40}, ...        % tabs on top, status bar at bottom
    "ColumnWidth", {'1x'}, ...
    "RowSpacing", 6, ...
    "Padding", [10 10 10 10]);
...
% ---- Shared status bar (always visible BELOW tabs) ----
hdr = uilabel(main, ...
    "Text", "  Initialising...", ...
    "FontSize", 15, "FontWeight", "bold", ...
    "BackgroundColor", [0.10 0.20 0.40], "FontColor", "w", ...
    "VerticalAlignment", "center");
hdr.Layout.Row = 2; hdr.Layout.Column = 1;
...
pb = uigridlayout(pbPanel, [2, 5], ...
    "RowHeight", {32, 50}, ...
    "ColumnWidth", {90, 90, 170, 200, 50, 90, '1x'}, ...
...
% Speed control: dropdown that picks how many frames advance per timer
% tick. 1× = normal (0.08 s per frame). 2× / 4× / 8× skip-render to get
% faster wall-clock playback during long runs.
speedLbl = uilabel(pb, "Text", "배속:", ...
    "FontWeight", "bold", "HorizontalAlignment", "right");
speedLbl.Layout.Row = 1; speedLbl.Layout.Column = 5;

speedDropdown = uidropdown(pb, ...
    "Items",     {'1x', '2x', '4x', '8x'}, ...
    "ItemsData", {1, 2, 4, 8}, ...
    "Value",     1, ...
    "Tooltip",   "타이머 1틱당 진행할 프레임 수. 높일수록 빠르게 재생.");
speedDropdown.Layout.Row = 1; speedDropdown.Layout.Column = 6;
...
pbSpacer.Layout.Row = 1; pbSpacer.Layout.Column = 7;
...
tab3Grid = uigridlayout(tab3, [3, 1], ...
    "RowHeight", {'1x', 50, '0.42x'}, ...   % shrink top, grow bottom so the
    ...                                       % test-cases panel doesn't show
    ...                                       % a large empty area at the top
...
replayInnerGrid = uigridlayout(replayInnerPanel, [3, 3], ...
    "RowHeight",   {42, 42, 30}, ...
    "ColumnWidth", {110, '1x', 170}, ...
    "Padding", [12 10 12 8], "RowSpacing", 8, "ColumnSpacing", 10);
...
btnReplay = uibutton(replayInnerGrid, "Text", "▶ 이 케이스 Replay", ...
    "BackgroundColor", [0.50 0.30 0.70], "FontColor", "w", ...
...
lblReplayStatus = uilabel(replayInnerGrid, ...
    ...
    "FontSize", 14, "FontColor", [0.30 0.25 0.15]);
...
% =========================================================================
    function onTick()
        ...
        % Read speed multiplier (1× / 2× / 4× / 8×) and advance that many
        % frames per timer tick — cheap fast-forward without retiming the
        % timer itself, so the boundary-search loop completes faster on
        % long sims.
        step = 1;
        try
            step = max(1, round(speedDropdown.Value));
        catch
        end
        state.frameIdx = state.frameIdx + step;
        if state.frameIdx > Nt
            state.frameIdx = Nt;          % clamp so renderFrame stays valid
            finalizeRun();
            return;
        end
    ...
    function onRunRequested()
        ...
        % ---
        % Reset case history at the start of a fresh session so the test-
        % case generation and LLM mission summary reflect ONLY the
        % scenario the user is about to run, not stale runs from previous
        % sessions. Auto-loop iterations (cooldown → next iter) call
        % startRun() directly and never come through here, so per-session
        % accumulation is preserved.
        state.history = struct( ...
            "iter", {}, "fog", {}, "ill", {}, "noi", {}, ...
            "metric", {}, "verdict", {}, ...
            "metric_person", {}, "metric_vehicle", {}, ...
            "ngt", {}, "ndet", {}, "ntp", {}, ...
            "n_intruders_total", {}, "n_intruders_seen", {}, ...
            "n_intruders_detected", {}, "n_intruders_missed", {}, ...
            "mode", {}, "analysis", {});
        state.iterCount = 0;
        % Force EO 1st-person renderer to fully rebuild intruder meshes on
        % the next frame so stale handles from the prior session don't
        % ghost-render alongside the freshly-walking persons.
        setappdata(0, 'EO_FORCE_INTRUDER_REDRAW', true);
        fprintf("[Run] state.history 초기화 — 새 boundary search 세션 시작.\n");
        if ~autoToggle.Value
    ...
    function recordReplayResult()
        ...
        try
            cmpPy = py.dashboard_step.compare_replay_result( ...
                ...
                last.metric);
            d = struct(cmpPy);
            % Pull saved expected metric + tolerance band for the
            % per-row table — accept_str joined into "PASS/MARGINAL/FAIL"
            % form for display.
            exp_m = NaN;
            try, exp_m = double(c.expected_metric); catch, end
            mn = NaN; mx = NaN;
            try, mn = double(c.metric_min); catch, end
            try, mx = double(c.metric_max); catch, end
            accept_str = "";
            try
                a = c.acceptable_verdicts;
                if iscell(a), a = string(a); end
                accept_str = strjoin(string(a), "/");
            catch
            end
            state.replayResults{end+1} = struct( ...
                ...
                "metric",     last.metric, ...
                "expected_metric", exp_m, ...
                "metric_min", mn, ...
                "metric_max", mx, ...
                "acceptable", accept_str, ...
    ...
    function html = replayResultsHtml(results, nOK, nDrift, nReg)
        % Format batch replay comparison result as an Excel-style HTML
        % table so each row's actual/expected/band/tag is scannable.
        if nReg > 0
        ...
        for k = 1:numel(results)
            r = results{k};
            if r.regression
                tag = '✗ REGRESSION'; tagCol = '#B5301A';
            elseif r.drift
                tag = '△ DRIFT'; tagCol = '#A56A18';
            else
                tag = '✓ OK'; tagCol = '#1A5028';
            end
            % Format numeric fields safely
            mActual = sprintf('%.3f', double(r.metric));
            if isnan(double(r.expected_metric))
                mExp = '—';
            else
                mExp = sprintf('%.3f', double(r.expected_metric));
            end
            if isnan(double(r.metric_min)) || isnan(double(r.metric_max))
                band = '—';
            else
                band = sprintf('[%.2f, %.2f]', ...
                    double(r.metric_min), double(r.metric_max));
            end
            accStr = char(r.acceptable);
            if isempty(accStr), accStr = '—'; end
            if mod(k, 2) == 1, bg = '#FFFFFF'; else, bg = '#F8FFF8'; end
            rows = [rows, ...                                          %#ok<AGROW>
                '<tr style="background:', bg, ';">', ...
                '<td style="text-align:center;font-weight:bold;">', num2str(r.idx), '</td>', ...
                '<td style="text-align:center;font-weight:bold;color:', tagCol, ';">', tag, '</td>', ...
                '<td style="text-align:center;font-weight:bold;">', char(r.actual), '</td>', ...
                '<td style="text-align:center;font-family:monospace;">', mActual, '</td>', ...
                '<td style="text-align:center;">', char(r.expected), '</td>', ...
                '<td style="text-align:center;font-family:monospace;">', mExp, '</td>', ...
                '<td style="text-align:center;font-size:13px;">', accStr, '</td>', ...
                '<td style="text-align:center;font-family:monospace;font-size:13px;">', band, '</td>', ...
                '</tr>'];
        end
        ...
        header = [...
            ...
            '<th style="padding:6px 8px;">Actual Verdict</th>', ...
            '<th style="padding:6px 8px;">Actual mAP</th>', ...
            '<th style="padding:6px 8px;">Expected Verdict</th>', ...
            '<th style="padding:6px 8px;">Expected mAP</th>', ...
            '<th style="padding:6px 8px;">허용 Verdict</th>', ...
            '<th style="padding:6px 8px;">허용 mAP 구간</th>', ...
            '</tr></thead>'];

        body = ['<p style="margin:0 0 8px 0;"><b style="font-size:18px;color:#1A5028;">', ...
                '▣ Replay 결과 — 총 ', num2str(numel(results)), ' cases</b></p>', ...
                '<p style="margin:0 0 6px 0;font-size:15px;">', ...
                '✓ OK ', num2str(nOK), '   ·   ', ...
                '△ DRIFT ', num2str(nDrift), '   ·   ', ...
                '✗ REGRESSION ', num2str(nReg), '</p>', ...
                '<p style="margin:0 0 14px 0;color:', statusCol, ';">', ...
                statusLine, '</p>', ...
